...
begin
  ...
  where au.id = any(p_user_ids)
    ...
    and exists (
      select 1 from public.business_members bm_caller
      join public.business_members bm_target
        on bm_target.business_id = bm_caller.business_id
      where bm_caller.user_id = auth.uid()
        and bm_target.user_id = au.id
    );
end;
$$;
